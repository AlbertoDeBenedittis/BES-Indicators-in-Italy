...
'''Get the lists with places\' names'''
...
for stat in List_Indicators: 
    ...
    if selectbox_area == 'Macroareas':
        if df_macro[y_].isnull().all() :
            st.error(('There are no data available for the chosen {indicator} in the selected year').format(indicator = name_data))
        else:
            ...
            try:
                folium_static(folium_interactive_map(df_macro, y_, stat, labels))
            except:
                st.write('No Folium Map avaialble')
        ...
        if df_reg[y_].isnull().all() :
            ...
            try:
                folium_static(folium_interactive_map(df_reg, y_, stat, labels))
            except:
                st.write('No Folium Map avaialble')
    else:
        if df_prov[y_].isnull().all() :
            ...
            try:    
                folium_static(folium_interactive_map(df_prov, y_, stat, labels))
            except:
                st.write('No Folium Map avaialble')